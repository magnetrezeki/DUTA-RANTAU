...
export async function POST(req:NextRequest){
 ...
 if(!isDutaAiEnabled())return NextResponse.json({error:'DUTA AI belum diaktifkan.',code:'AI_DISABLED'},{status:503});
 ...
 const quota=consumeDutaAiQuota(ip);if(!quota.allowed)return NextResponse.json({error:'Kuota penggunaan wajar DUTA AI telah dicapai. Sila cuba semula selepas tetapan semula.',code:'QUOTA_EXCEEDED',quota},{status:429});
