...
export type DutaToolResult = { tool: DutaToolName; status: "SUCCESS" | "EMPTY" | "UNAVAILABLE"; items: Array<{ title: string; reference?: string; sourceType: string; trustLevel: TrustLevel; lastVerifiedAt?: string }>; resultCount: number; errorCategory?: "UNAVAILABLE" | "EMPTY" };
...
export async function runDutaTool(message: string): Promise<DutaToolResult> {
  ...
  try {
    ...
    if (tool === "search_jobs") { const rows = await withPublicTransaction(tx => tx.select().from(jobs).where(eq(jobs.recordStatus, "ACTIVE")).orderBy(desc(jobs.createdAt)).limit(5)) as Array<typeof jobs.$inferSelect>; const items = rows.filter(x => matches(`${x.title} ${x.employer} ${x.city ?? ""} ${x.state ?? ""}`, message) || /kerja|lowongan|jawatan|operator|kilang/.test(message.toLowerCase())).map(x => ({ title: `${x.title} — ${x.employer}`, sourceType: "DUTA listing", trustLevel: trust(x.trustLevel) })); return { tool, status: items.length ? "SUCCESS" : "EMPTY", items, resultCount: items.length, ...(items.length ? {} : { errorCategory: "EMPTY" as const }) }; }
