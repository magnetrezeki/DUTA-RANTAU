...
export type AiTelemetryEvent = { correlationId?:string; userRef: string; intent: string; risk: string; sensitivity: Sensitivity; modelClass: ModelClass; provider: string | null; model: string | null; sourceRequirement: SourceRequirement; sourceTier: SourceTier | null; quotaOutcome: 'allowed'|'denied'|'error'|'not_applicable'; weightedUnits: number; inputTokens: number; outputTokens: number; estimatedCostUsd: number; latencyMs: number; success: boolean; errorCode: string | null; fallbackUsed: boolean };
