...
describe('DUTA AI policy foundation',()=>{
 ...
 it('keeps the deprecated compatibility meter aligned to the 30-unit UTC-day policy',()=>{const a=consumeDutaAiQuota('test-a',0);expect(a).toMatchObject({allowed:true,limit:30,remaining:29});let result=a;for(let i=0;i<29;i++)result=consumeDutaAiQuota('test-a',0);expect(result.allowed).toBe(true);expect(consumeDutaAiQuota('test-a',0).allowed).toBe(false);});
 it('resets the compatibility meter at the next UTC day rather than after one hour',()=>{const lateUtc=Date.UTC(2026,0,1,23,59);expect(consumeDutaAiQuota('utc-boundary',lateUtc).resetAt).toBe(new Date(Date.UTC(2026,0,2)).toISOString());expect(consumeDutaAiQuota('utc-boundary',Date.UTC(2026,0,2))).toMatchObject({allowed:true,remaining:29});});
