...
describe("V2.5 AI and voice integration security", () => {
  ...
  it("keeps ASR failure separate from text capability", async () => { delete process.env.OPENAI_TRANSCRIBE_MODEL; await expect(getASRProvider().transcribe({ audio: new File(["audio"], "voice.webm", { type: "audio/webm" }) })).resolves.toMatchObject({ success: false, provider: "fallback" }); await expect(getAIProvider().generate({ message: "Saya akan mengetik", channel: "text" })).resolves.toMatchObject({ provider: "fallback" }); });
  ...
  it("does not persist audio or expose provider secrets through contracts", () => { expect(JSON.stringify(getTTSProvider())).not.toContain("NVIDIA_API_KEY"); expect(JSON.stringify(getASRProvider())).not.toContain("OPENAI_API_KEY"); });
