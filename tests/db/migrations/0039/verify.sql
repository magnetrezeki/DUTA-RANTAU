...
BEGIN
  ...
  SELECT pg_get_expr(d.adbin,d.adrelid) INTO default_expression FROM pg_attrdef d JOIN pg_attribute a ON a.attrelid=d.adrelid AND a.attnum=d.adnum WHERE d.adrelid='public.official_sources'::regclass AND a.attname='source_purpose';
  PERFORM pg_temp.assert_true(default_expression IS NULL OR default_expression ~* 'null', 'source_purpose no non-null default');
  ...
  PERFORM pg_temp.assert_true(EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.official_source_governance'::regclass AND conname='official_source_governance_verification_metadata_ck' AND pg_get_constraintdef(oid) ~ 'verified_at IS NOT NULL' AND pg_get_constraintdef(oid) ~ 'verified_by IS NOT NULL') AND EXISTS (SELECT 1 FROM pg_constraint WHERE conrelid='public.official_source_governance'::regclass AND conname='official_source_governance_approval_metadata_ck' AND pg_get_constraintdef(oid) ~ 'approved_at IS NOT NULL' AND pg_get_constraintdef(oid) ~ 'approved_by IS NOT NULL'), 'governance checks');
