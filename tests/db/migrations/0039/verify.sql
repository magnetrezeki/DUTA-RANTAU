...
BEGIN
  ...
  PERFORM pg_temp.assert_true((SELECT array_agg(a.attname::text ORDER BY a.attnum) FROM pg_index i JOIN pg_attribute a ON a.attrelid=i.indrelid AND a.attnum=ANY(i.indkey) WHERE i.indrelid='public.official_source_governance'::regclass AND i.indisprimary)=ARRAY['source_id'], 'governance primary key');
  ...
  PERFORM pg_temp.assert_true((SELECT array_agg(a.attname::text ORDER BY a.attnum) FROM pg_index i JOIN pg_attribute a ON a.attrelid=i.indrelid AND a.attnum=ANY(i.indkey) WHERE i.indrelid='public.official_source_evidence'::regclass AND i.indisprimary)=ARRAY['id'], 'evidence primary key');
  ...
  PERFORM pg_temp.assert_true((SELECT array_agg(attname::text ORDER BY attnum) FROM pg_attribute WHERE attrelid='public.official_sources'::regclass AND attnum>0 AND NOT attisdropped AND has_column_privilege('duta_app','public.official_sources',attname,'INSERT'))=ARRAY['institution','channel','url','category','priority','trust_level','last_checked','checksum','active'] AND (SELECT array_agg(attname::text ORDER BY attnum) FROM pg_attribute WHERE attrelid='public.official_sources'::regclass AND attnum>0 AND NOT attisdropped AND has_column_privilege('duta_app','public.official_sources',attname,'UPDATE'))=ARRAY['priority','trust_level','active'], 'source column privileges');
